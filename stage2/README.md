...
请在你的 `project_root` 下更新目录结构。我们将遵循 **“理论验证 + 框架实战”** 的双轨制。

```text
project_root/
|-- stage2/
|   |-- README.md                    (本阶段总体规划)
|   |-- week5_theory/                (Week 5: 手撕 Transformer 核心组件)
|   |   |-- S2W5D1_Positional_Encoding.ipynb
|   |   |-- S2W5D2_Self_Attention.ipynb  <-- 核心中的核心
|   |   |-- S2W5D3_MultiHead_Block.ipynb
|   |   |-- S2W5D6_ViT_Bridge.ipynb      <-- 具身智能迁移视角
|   |-- week6_hf_tools/              (Week 6: Hugging Face 工具链熟悉)
|   |   |-- data/                    (存放 NLP 练习数据)
|   |   |-- demo_tokenizer.ipynb
|   |-- week7_project/               (Week 7: 简历项目 - BERT 微调)
|   |   |-- train_bert.py            (微调脚本)
|   |   |-- inference.py             (推理脚本)
```
...
黄组长，Stage 2 的蓝图已经绘就。我们要开始啃最硬的骨头了。

**Would you like me to start with "Week 5 Day 1: Input Embedding & Positional Encoding"?**

(我会结合 PyTorch 代码和图解，并解释为什么机器人领域（如 Transformer-based Policy）也必须用这个。)